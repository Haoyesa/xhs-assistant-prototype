...
3. **发布**：打开创作者发布台(`creator.xiaohongshu.com`)→ 右下角「🚀 发布助手」悬浮面板（或扩展弹窗）点「开始批量发布」自动逐篇填充标题/正文/话题/配图/关联商品并自动发布。多账号（比特浏览器）模式下需先在后端「发布→任务指派」把笔记指派给本窗口绑定的账号；面板按钮按本账号统计队列，**发布中或本账号无待发任务时自动禁用**。
